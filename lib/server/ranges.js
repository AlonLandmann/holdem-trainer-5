...
import { spotInfo } from '../shared/spots'
import { computeComplexity } from '../client/stats'
...
export function actionToStorage(action) {
  switch (action.type) {
    ...
    case 'flop': return `flop-${action.cards[0]}-${action.cards[1]}-${action.cards[2]}`
    case 'flop': return `turn-${action.cards[0]}`
    case 'flop': return `river-${action.cards[0]}`
  }
}
...
export function optionToStorage(option) {
  switch (option.type) {
    case 'fold': return `fold`
    case 'call': return `call`
    case 'check': return `check`
    case 'bet': return `bet-${option.size}`
    case 'raise': return `raise-${option.size}`
  }
}
...
export function toClientFormat(range) {
  ...
  copy.complexity = computeComplexity(copy)
  return copy
}

...
export function outputColor(actions, toCallNumbers, strategy) {
  const hues = []

  for (let i = 0; i < actions.length; i++) {
    if (toCallNumbers[i] > 0) {
      if (actions[i] == 0) {
        hues.push(FOLD_HUE)
      } else if (actions[i] == toCallNumbers[i]) {
        hues.push(CHECK_OR_CALL_HUE)
      } else {
        hues.push(hueFromSize((actions[i] - toCallNumbers[i]) / toCallNumbers[i]))
        // ABOVE IS INCORRECT
      }
    } else if (toCallNumbers[i] == 0) {
      if (actions[i] == 0) {
        hues.push(CHECK_OR_CALL_HUE)
      } else {
        hues.push(hueFromSize(actions[i]))
        // ABOVE IS INCORRECT
      }
    }
  }

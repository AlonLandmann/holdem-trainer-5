...
export default async function handleUserRequest(req, res, method, f) {
  try {
    ...
    let user = !sessionId ? null : await prisma.user.findFirst({
      where: {
        session: {
          is: {
            token: sessionId
    ...
    const { success, message } = await f(prisma, user)

    if (!success) {
      return res.status(500).json({ success: false, message: 'Something went wrong.' })
    }

    user = !sessionId ? null : await prisma.user.findFirst({
      where: {
        session: {
          is: {
            token: sessionId
    ...
    return res.status(200).json({ success, message, updatedUser: user })
  } catch (error) {
    console.log(error)

import { values, cardsFromCombo, isValid, numFromValue, numFromSuit } from '@/lib/cards'
import { reverse, sortBy } from 'lodash'

export function categorize(range) {
  let result = {
    all: [],
    allStrats: Array(range.options.length).fill(0),
    strengths: {},
    strengthStrats: {},
    details: { '1pr': {}, 'HC': {} },
    detailStrats: { '1pr': {}, 'HC': {} },
    detailSortValues: { '1pr': {}, 'HC': {} },
    draws: {},
    drawStrats: {}
  }

  range.matrix.forEach(({ combo, frequency, strategy }) => {
    if (!isValid(combo, range.spot.board)) {
      return null
    }
    ...
    if (range.spot.board.length < 3) {
      return null
    }

    const { strength, detail, draw: { flush, straight } } = categorizeCombo(combo, range.spot.board)
    ...
    if (['1pr', 'HC'].includes(strength)) {
      if (!(detail in result.details[strength])) {
        result.details[strength][detail] = [combo]
        result.detailSortValues[strength][detail] = detailSortValue(combo, range.spot.board, strength)
...
export function retrieveInOrder(categories, type, strength) {
  const strengthOrder = ['SF', 'Quads', 'FH', 'Flush', 'Straight', 'Trips', '2pr', '1pr', 'HC']
  const drawOrder = ['FD', 'BDFD', 'NFD', 'OE', 'Gut', 'NSD']
...
export function categorizePreflop(range) {
  let result = {
    all: [],
    allStrats: Array(range.options.length).fill(0),
    strengths: {},
...
function detectFlushDraw(s, strength) {
  if (['SF', 'Quads', 'FH', 'Flush'].includes(strength)) return 'NFD'
  const maxSuitCount = flushCount(s)
  if (maxSuitCount == 4) return 'FD'
  if (maxSuitCount == 3 && s.length == 5) return 'BDFD'
  return 'NFD'
}
...
function detectStraightDraw(v, strength) {
  if (['SF', 'Quads', 'FH', 'Flush', 'Straight'].includes(strength)) return 'NSD'
  let count = 0
  for (let i = 2; i <= 14; i++) {
...
function detectParity(v) {
  ...
  if (max == 3 && countsArray.length == 2) return 'FH'
  if (max == 3 && countsArray.length != 2) return 'Trips'
  if (max == 2 && countsArray.length == 3) return '2pr'
  if (max == 2 && countsArray.length != 3) return '1pr'
  if (max == 1) return 'HC'
}
...
function determineFiveCardStrength(v, s) {
  ...
  if (hasFlush && hasStraight) return { name: 'SF', sortValue: 9 }
  if (parity == 'Quads') return { name: 'Quads', sortValue: 8 }
  if (parity == 'FH') return { name: 'FH', sortValue: 7 }
  ...
  if (parity == '2pr') return { name: '2pr', sortValue: 3 }
  if (parity == '1pr') return { name: '1pr', sortValue: 2 }
  if (parity == 'HC') return { name: 'HC', sortValue: 1 }
}
...
function detailSortValue(combo, board, strength) {
  if (strength != '1pr' && strength != 'HC') return null
  const cV = getOrderedValues(cardsFromCombo(combo))
  const bV = getOrderedValues(board)
  if (strength == '1pr' && bV.includes(cV[0])) return cV[0]
  if (strength == '1pr' && bV.includes(cV[1])) return cV[1]
  return cV[0]
}
